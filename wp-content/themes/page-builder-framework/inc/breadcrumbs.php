...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the WPBF_Breadcrumbs class,
...
class WPBF_Breadcrumbs {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `WPBF_Breadcrumbs::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see WPBF_Breadcrumbs::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see WPBF_Breadcrumbs::set_post_taxonomy()
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="wpbf-breadcrumbs" itemscope="itemscope" itemtype="https://schema.org/BreadcrumbList">',
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="wpbf-breadcrumbs-wrapper" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'wpbf_breadcrumbs', $breadcrumb, $this->args );
